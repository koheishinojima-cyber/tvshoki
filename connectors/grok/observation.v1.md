...
The machine-validatable schema is [`observation.v1.schema.json`](observation.v1.schema.json). The JSON Schema block below must stay in sync with that file. Fixtures and a minimal validator live beside this contract; they are not live samples.
...
| `raw_text` | string | **Verbatim source quote** (caption, post body, page extract, transcript snippet). Copy as observed. Do not rewrite it into a claim, and do not strip tokens to satisfy the forbidden policy |
...
| Key | Type | Notes |
| --- | --- | --- |
| `url` | string | Grok-normalized source URL. Do not write Rakuten URLs here. If the source quote contains a Rakuten URL, leave it inside `raw_text` |
...
Optional object. Include **only fields that are known**. `additionalProperties: false`. Do not guess network, air time, or episode title. Omit `program` entirely when the source is not one of the official programs below.

| Key | Type | Notes |
| --- | --- | --- |
| `name` | string | Enum: `シューイチ` \| `サタデープラス`. If present, must be exactly one of these |
...
`program.name` is a closed enum:

- `シューイチ`
- `サタデープラス`

Do not substitute nicknames, hashtags, or unofficial shortenings. If the source is not clearly one of these programs, omit `program` (or omit `name`) rather than approximating.
...
Each element is an object, `additionalProperties: false`.

**Required on every candidate:** `claim_level`, `evidence`.

**At least one of** these identity fields must be present and non-empty: `brand`, `product_name`, `model_number`, `jan`, `category`. Encoded in the Schema as `anyOf` + `minLength: 1`.
...
| `claim_level` | string | **Required.** Enum: `confirmed_on_air` \| `weak_confirmation` \| `inferred` |
| `confidence` | number | Closed interval **0–1** (inclusive). Observation confidence only — not an OS score |
| `evidence` | string | **Required.** Why this candidate exists: quote, timestamp, or page locus. Grok-generated; do not put Rakuten URLs here |
...
Never write `愛用` (or equivalent habitual-use wording) in **Grok-generated** fields (`evidence`, candidate names that Grok paraphrases, comments) unless **primary** on-air or official program-page evidence supports that exact claim. A post, roundup, or rumor that someone "uses" a product is not primary evidence of 愛用.

If the source quote itself contains `愛用`, leave it unchanged in `raw_text`.

## Forbidden (Grok-generated fields only)

The following must **not** appear in Grok-generated **normalized** fields or **judgment** fields: `source.url`, `program.*`, `product_candidates.*` (including `evidence`, `claim_level`, `confidence`), extra keys, side-channel notes.

- Rakuten URLs
- OS scores
- `公開してよい` / publish judgments
- EPC
- click counts

`raw_text` is a verbatim source field. **Do not rewrite `raw_text` to strip forbidden content.** Schema validation does not scan `raw_text` for these strings. A source quote may mention Rakuten, EPC, click counts, or 「公開してよい」; that is evidence of what was said, not a Grok judgment.
...
Canonical copy: [`observation.v1.schema.json`](observation.v1.schema.json). Keep this block identical.
...
```json
{
  ...
  "raw_text": "<verbatim source quote; do not strip>",
  "program": {
    "name": "シューイチ | サタデープラス",
  ...
  "product_candidates": [
    {
      "brand": "<at least one identity field>",
      "product_name": "<optional if another identity field is set>",
      "model_number": "<optional if another identity field is set>",
      "jan": "<optional if another identity field is set>",
      "category": "<optional if another identity field is set>",
      "claim_level": "confirmed_on_air | weak_confirmation | inferred",
      "confidence": 0.0,
      "evidence": "<required; Grok-generated; no Rakuten URL>"
    }
  ]
}
```
